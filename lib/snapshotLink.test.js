...
describe('snapshotLink', () => {
    ...
    it('uses the Surveillance Station 9 endpoint for H.265', () => {
        ...
        expect(link).to.equal('https://nas:5001/webapi/entry.cgi?api=SYNO.SurveillanceStation.Camera&method=GetSnapshot&version=9&id=42&profileType=0&_sid=sid');
    });

    it('uses the Surveillance Station 9 endpoint for H.265+', () => {
        const link = createSnapshotLink({ protocol: 'https', host: 'nas', port: 5001, sessions: { SurveillanceStation: { _sid: 'sid' } } }, 42, 'H265+');
        expect(link).to.include('version=9&id=42&profileType=0');
    });
