...
function createSnapshotLink(syno, cameraId, videoCodec, camera, config, namespace) {
    ...
    if (reolinkConfig && reolinkConfig.login && reolinkConfig.password && camera && camera.host) {
        ...
        const url = `${protocol}://${camera.host}:${port}/cgi-bin/api.cgi?cmd=Snap&channel=${channel}&rs=ioBroker&user=${encodeURIComponent(reolinkConfig.login)}&password=${encodeURIComponent(reolinkConfig.password)}`;
        return namespace ? {local: true, url} : url;
    }
    ...
    if (videoCodec === 'H265' || videoCodec === 'H265+') {
        return `${syno.protocol}://${syno.host}:${syno.port}/webapi/entry.cgi?api=SYNO.SurveillanceStation.Camera&method=GetSnapshot&version=9&id=${cameraId}&profileType=0&_sid=${sid}`;
    }
