...
function createSnapshotLink(syno, cameraId, videoCodec, camera, config) {
    ...
    const vendor = `${camera && camera.vendor || ''} ${camera && camera.model || ''}`.toLowerCase();
    if ((videoCodec === 'H265' || videoCodec === 'H265+') && vendor.includes('reolink') && config && config.reolinkLogin && config.reolinkPassword && camera && camera.host) {
        const protocol = config.reolinkHttps ? 'https' : 'http';
        const port = camera.port || (config.reolinkHttps ? 443 : 80);
