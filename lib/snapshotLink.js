...
function createSnapshotLink(syno, cameraId, videoCodec, camera, config) {
    ...
    if (config && camera && camera.name && Array.isArray(config.snapshotCameras)) {
        reolinkConfig = config.snapshotCameras.find(item => {
            if (!item || item.source !== 'reolink') return false;
            // objectId stores the complete ioBroker object ID, while older
            // configurations stored only the camera name.
            const configuredCamera = typeof item.camera === 'string' && item.camera.includes('.SurveillanceStation.cameras.')
